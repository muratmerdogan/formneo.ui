...
function UserProjects() {
  ...
  const fetchDepartmentsData = async () => {
    try {
      ...
      let response = await api.apiTicketDepartmentsAllOnlyNameGet();
  ...
  const columns = [
    ...
    {
      accessor: "name", // aslında row.original'da subname var
      Header: (
        <div style={{ fontSize: "16px", fontWeight: "bold", color: "black" }}>Proje Tanımı</div>
      ),
      Cell: ({ row }: any) => {
        const { name, subProjectName } = row.original;
        return (
          <div>
            {name} {subProjectName && <span> - {subProjectName}</span>}
          </div>
        );
      },
    },
    {
      ...
      Cell: ({ value }: any) => {
        if (!value) return null; // manager yoksa boş bırak
        ...
        titleArea={
          ...
          >
            <MDBox>
              ...
              >
                Kullanıcıların projelerini görüntüleyin
              </Typography>
            </MDBox>
          </ObjectPageTitle>
        }
